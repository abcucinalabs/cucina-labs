...
export function HomeCarousel() {
  ...
  const slides = [
    {
      ...
      content: (
        <>
          <p className="text-sm sm:text-base md:text-lg lg:text-xl text-white/75 mb-8 sm:mb-10 lg:mb-12 leading-relaxed max-w-xl">
  ...
  return (
    <div className="relative w-full flex justify-center">
      {/* Card Container */}
      <div className="w-full max-w-[768px] rounded-[var(--radius-2xl)] border border-white/15 bg-[#0d0d0d]/56 p-6 sm:p-8 md:p-10 lg:p-12 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] backdrop-blur-2xl relative overflow-hidden">
        ...
        {/* Slide Content */}
        <div className="relative z-10 min-h-[400px] sm:min-h-[380px] md:min-h-[350px]">
          {slides.map((slide, index) => (
            ...
            >
              <div className="inline-flex items-center gap-2 rounded-full border border-[rgba(60,53,242,0.4)] bg-[linear-gradient(135deg,rgba(60,53,242,0.18),rgba(74,95,217,0.18))] px-3 py-1.5 sm:px-4 sm:py-2 text-[10px] sm:text-xs font-medium uppercase tracking-[0.18em] text-[color:var(--accent-primary)] mb-4 sm:mb-6">
                <span className="h-1.5 w-1.5 sm:h-2 sm:w-2 rounded-full bg-[var(--accent-primary)] animate-pulse" />
                {slide.badge}
              </div>
        ...
        <div className="relative z-10 flex items-center justify-between mt-6 sm:mt-8">
          <button
            onClick={prevSlide}
            className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 flex items-center justify-center transition-all duration-300 hover:-translate-x-0.5 flex-shrink-0"
            ...
            {slides.map((_, index) => (
              <button
                ...
                className={`transition-all duration-300 rounded-full ${
                  currentSlide === index
                    ? "w-6 h-1.5 sm:w-8 sm:h-2 bg-[var(--accent-primary)]"
